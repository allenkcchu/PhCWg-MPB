...
s1yList = np.linspace(-25,25,11)-180
s2x = 0
s2y = 0

C = plt.cm.jet(np.linspace(0,1,len(s1xList)))
...
for s1y in s1yList:
    ...
    for M, s1x in enumerate(s1xList):
        s1x = int(s1x)
        s1y = int(s1y)
        ...
        for N in range(np.size(dfb,1)-5):
            ...
            if N in selectedBand:
                if N == selectedBand[0]:
                    ax[0].plot(dfb[' k1'],dfb[f' te band {N+1}'],color=C[M],label=f's1x={s1x}nm')
                else:
    ...
    fig.suptitle(f'Band10(11), a={a}nm, r={r}nm, s1y={s1y}nm, s2x={s2x}nm, s2y={s2y}nm')
    fig.tight_layout()
