...
FigPath = os.path.join(os.getcwd(),'s2shiftSweep','Figures')
DataPath = os.path.join(os.getcwd(),'s2ShiftSweep','Data')
...
C = plt.cm.jet(np.linspace(0,1,len(s2xList)))
...
for s2y in s2yList:
    ...
    for M, s2x in enumerate(s2xList):
        s2x = int(s2x)
        s2y = int(s2y)
        ...
        for N in range(np.size(dfb,1)-5):
            ...
            if N in selectedBand:
                if N == selectedBand[0]:
                    ax[0].plot(dfb[' k1'],dfb[f' te band {N+1}'],color=C[M],label=f's2x={s2x}nm')
                else:
    ...
    fig.suptitle(f'Band10(11), a={a}nm, r={r}nm, s1x = s1x{s1x}nm, s1y={s1y}nm, s2y={s2y}nm')
    fig.tight_layout()
